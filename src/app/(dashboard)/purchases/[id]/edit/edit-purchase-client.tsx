...
import { ArrowLeft, Save, Plus, Trash2, Search, Calendar, Loader2, AlertTriangle } from 'lucide-react'
...
interface CartItem {
  id: string
  itemDbId?: string // purchase_items.id if existing
  product: Product | null
  originalQty: number // qty sebelum edit, untuk revert stok
  qty: number
  unit_price: number
  subtotal: number
}

interface EditPurchaseClientProps {
  purchase: any
  products: Product[]
  suppliers: Supplier[]
  warehouses: any[]
...
function ProductCombobox({ 
  item, products, isActive, onActivate, onDeactivate, onSelect 
}: {
  item: CartItem; products: Product[]; isActive: boolean;
  onActivate: () => void; onDeactivate: () => void; onSelect: (p: Product) => void;
}) {
  ...
  if (!isActive && item.product) {
    return (
      <div className="flex flex-col cursor-pointer p-2 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-dark-200 w-full text-left" onClick={onActivate}>
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      <div className="flex items-center bg-white border border-primary-300 rounded-lg overflow-hidden shadow-sm ring-2 ring-primary-500/20">
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-dark-100 rounded-xl shadow-xl max-h-60 overflow-y-auto z-50 animate-slide-up">
          <div className="p-1">
            {filtered.length === 0 ? (
              ...
              filtered.map(p => (
                <div key={p.id} className="flex flex-col p-2.5 hover:bg-primary-50 rounded-lg cursor-pointer transition-colors border-b border-dark-50 last:border-0"
...
export default function EditPurchaseClient({ purchase, products, suppliers, warehouses, userId, branchId, defaultWarehouseId }: EditPurchaseClientProps) {
  ...
  const [paymentStatus, setPaymentStatus] = useState<'lunas' | 'tempo'>(purchase.payment_status)
  const [paymentMethod, setPaymentMethod] = useState<'tunai' | 'transfer' | 'qris' | 'tempo'>(purchase.payment_method || 'tunai')
  const [notes, setNotes] = useState(purchase.notes || '')
  const [loading, setLoading] = useState(false)
  const [activeProductSearchId, setActiveProductSearchId] = useState<string | null>(null)

  // Build initial items from existing purchase_items
  const [items, setItems] = useState<CartItem[]>(() => {
    const existingItems = (purchase.purchase_items || []).map((pi: any) => ({
      id: pi.id,
      itemDbId: pi.id,
      product: pi.product,
      originalQty: pi.qty,
    ...
    return [...existingItems, { id: `new-${Date.now()}`, product: null, originalQty: 0, qty: 1, unit_price: 0, subtotal: 0 }]
  })

  const addEmptyRow = () => {
    setItems(prev => [...prev, { id: `new-${Date.now()}`, product: null, originalQty: 0, qty: 1, unit_price: 0, subtotal: 0 }])
  }
  ...
  const removeRow = (id: string) => {
    if (items.length > 1) {
      ...
      setItems([{ id: `new-${Date.now()}`, product: null, originalQty: 0, qty: 1, unit_price: 0, subtotal: 0 }])
    }
  }
  ...
  const selectProduct = (rowId: string, product: Product) => {
    setItems(items.map(item => {
      if (item.id === rowId) {
        ...
        const unit_price = product.hpp > 0 ? product.hpp : 0
        return { ...item, product, unit_price, subtotal: qty * unit_price }
      }
      return item
    }))
    setActiveProductSearchId(null)
    if (items[items.length - 1].id === rowId) addEmptyRow()
  }

  const updateItemQty = (rowId: string, qty: number) => {
    setItems(items.map(item => item.id === rowId ? { ...item, qty, subtotal: qty * item.unit_price } : item))
  }

  const updateItemPrice = (rowId: string, unit_price: number) => {
    setItems(items.map(item => item.id === rowId ? { ...item, unit_price, subtotal: item.qty * unit_price } : item))
  }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      for (const oldItem of (purchase.purchase_items || [])) {
        if (!oldItem.product_id) continue
        // Get current stock
        const { data: currentProduct } = await supabase
          .from('products').select('stock_quantity').eq('id', oldItem.product_id).single()
        if (currentProduct) {
      ...
      for (const item of validItems) {
        ...
        const { data: currentProduct } = await supabase
          .from('products').select('stock_quantity').eq('id', item.product.id).single()
        const currentQty = currentProduct?.stock_quantity || 0
        await supabase.from('products').update({
          stock_quantity: currentQty + item.qty,
          hpp: item.unit_price
        ...
        await supabase.from('stock_adjustments').insert([{
          ...
          qty_before: bQty,
          qty_change: item.qty,
          qty_after: bQty + item.qty,
          reason: `Edit Pembelian: ${invoiceNumber}`,
      ...
      toast.error(error.message || 'Terjadi kesalahan saat memperbarui pembelian')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pb-24">
      {/* Top Navigation */}
      <div className="bg-white border-b border-dark-100 px-6 py-4 flex items-center justify-between sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-dark-900">Edit Faktur Pembelian</h1>
            <p className="text-sm text-dark-500 font-mono">{purchase.invoice_number}</p>
          </div>
        </div>
        
        <div className="flex items-center gap-3">
          {purchase.payment_status === 'lunas' && (
            <div className="flex items-center gap-1.5 text-xs text-warning-700 bg-warning-50 border border-warning-200 px-3 py-2 rounded-lg">
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>Pembelian sudah lunas — perubahan stok akan direkam ulang</span>
            </div>
          )}
          <button onClick={() => router.push('/purchases')} className="btn-md btn-outline bg-white">Batal</button>
          <button 
      ...
      <div className="max-w-6xl w-full mx-auto p-6 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-dark-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <div className="form-group">
              ...
              <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="input bg-white font-medium">
            ...
            <div className="form-group">
              ...
              <input type="text" value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)}
                className="input font-mono text-dark-900" placeholder="Contoh: INV/2023/001" />
            </div>
            <div className="form-group">
              ...
              <div className="relative">
                ...
                <input type="date" value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} className="input pl-10" />
              </div>
            </div>
            <div className="form-group">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-dark-100 overflow-hidden flex flex-col">
                ...
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-3 align-top">
                      <input type="number" min="1" className="input py-2 text-center"
                        value={item.qty || ''} onChange={(e) => updateItemQty(item.id, Number(e.target.value))}
                    ...
                    <td className="p-3 align-top">
                      <div className="relative">
                        ...
                        <input type="number" min="0" className="input py-2 pl-9"
                          value={item.unit_price || ''} onChange={(e) => updateItemPrice(item.id, Number(e.target.value))}
                          disabled={!item.product} />
                      </div>
                    </td>
                    <td className="p-3 align-top text-right pt-5 font-bold text-dark-900">{formatRupiah(item.subtotal)}</td>
          ...
          <div className="p-4 border-t border-dark-100 bg-white flex justify-center">
            <button onClick={addEmptyRow} className="btn-sm btn-outline text-primary-600 border-primary-200 hover:bg-primary-50 px-6">
              <Plus className="w-4 h-4 mr-1.5" /> Tambah Baris Kosong
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex-1 w-full bg-white p-5 rounded-2xl shadow-sm border border-dark-100">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              className="input resize-none h-24" placeholder="Tulis keterangan atau catatan khusus..." />
          </div>
          <div className="w-full md:w-[400px] bg-dark-900 p-6 rounded-2xl shadow-lg text-white">
            ...
            <div className="pt-6 border-t border-white/10 flex justify-between items-end">
              ...
              <span className="text-3xl font-black text-primary-400 tracking-tight">{formatRupiah(totalAmount)}</span>
            </div>
          </div>
        </div>
      </div>
