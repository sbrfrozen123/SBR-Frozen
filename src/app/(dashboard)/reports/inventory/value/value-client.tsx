...
import { Printer, Download, RefreshCw, Settings2, X, MapPin, ChevronLeft } from 'lucide-react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { formatRupiah } from '@/lib/utils/currency'
import { cn } from '@/lib/utils/cn'

interface ValueClientProps {
  products: any[]
  branches: any[]
  isLoaded: boolean
...
export default function ValueClient({
  ...
  const [showModal, setShowModal] = useState(!isLoaded)
  ...
  const applyFilter = () => {
    setShowModal(false)
    let url = `/reports/inventory/value?load=true`
  ...
  const exportToCSV = () => {
    const headers = ['Kode Barang', 'Nama Barang', 'Gudang/Cabang', 'Sisa Stok', 'Batas Minimum', 'UoM', 'HPP', 'Total Nilai']
    ...
    link.setAttribute('download', `Nilai_Persediaan_Barang_${asOfDate.replace(/ /g, '_')}.csv`)
    ...
    <div className="bg-slate-200 min-h-screen flex flex-col items-center py-8 relative font-sans print:bg-white print:py-0 print:block">
      ...
      <div className="bg-white shadow-xl max-w-[1000px] w-full mx-4 min-h-[800px] p-12 print:shadow-none print:m-0 print:max-w-none print:p-0">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-[#800000] mt-1">Laporan Nilai Persediaan Barang</h1>
            ...
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-t-2 border-b-2 border-dark-900 text-dark-900">
                  ...
                  <th className="py-3 px-2 font-bold text-right whitespace-nowrap">Sisa Stok</th>
                  <th className="py-3 px-2 font-bold whitespace-nowrap">UoM</th>
                  <th className="py-3 px-2 font-bold text-right whitespace-nowrap">HPP</th>
                  <th className="py-3 px-2 font-bold text-right whitespace-nowrap">Total Nilai</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-dark-200 text-dark-700">
                {products.map((item, idx) => (
                  <tr key={`${item.id}-${idx}`} className="hover:bg-slate-50 print:hover:bg-transparent transition-colors">
                    <td className="py-3 px-2 font-mono text-dark-500">{item.sku}</td>
                    <td className="py-3 px-2 font-medium">{item.name}</td>
                    <td className="py-3 px-2">{item.branch_name}</td>
                    <td className="py-3 px-2 text-right">
                      <span className={cn(
                        "font-semibold",
                        item.stock_quantity <= item.min_stock ? "text-danger-600" : ""
                      )}>
                        {item.stock_quantity}
                      </span>
                    </td>
                    <td className="py-3 px-2">{item.unit}</td>
                    <td className="py-3 px-2 text-right">{formatRupiah(item.hpp).replace('Rp', '')}</td>
                    <td className="py-3 px-2 text-right font-semibold">{formatRupiah(item.hpp * item.stock_quantity).replace('Rp', '')}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-b-2 border-dark-900 font-bold text-dark-900">
                  <td colSpan={6} className="py-3 px-2 text-right">TOTAL NILAI ASET KESELURUHAN</td>
                  <td className="py-3 px-2 text-right">{formatRupiah(products.reduce((sum, item) => sum + (item.hpp * item.stock_quantity), 0)).replace('Rp', '')}</td>
                </tr>
              </tfoot>
            </table>
          )}
        ...
        <div className="fixed inset-0 bg-dark-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 print-hidden animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-slide-up">
            <div className="bg-[#1a365d] text-white px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => isLoaded ? setShowModal(false) : null} className="text-white/70 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                <h4 className="text-lg text-dark-700 mb-4 border-b border-dark-200 pb-2">Tanggal Laporan</h4>
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <label className="w-24 text-sm text-dark-700">Per Tanggal</label>
                    <div className="flex-1 relative">
                      <input 
                        type="text" 
                        value={asOfDate}
                        disabled
                        className="w-full border-dark-200 rounded-lg text-sm pl-3 py-2 bg-slate-100 text-dark-500 font-medium cursor-not-allowed"
                      />
                      <p className="text-[10px] text-dark-400 mt-1">*Nilai inventaris selalu menampilkan data saat ini (real-time).</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-dark-200 flex justify-end">
              <button 
                onClick={applyFilter}
                className="bg-[#1a365d] hover:bg-[#12284c] text-white px-6 py-2 rounded text-sm font-bold transition-colors"
              >
                Tampilkan
              </button>
            </div>
