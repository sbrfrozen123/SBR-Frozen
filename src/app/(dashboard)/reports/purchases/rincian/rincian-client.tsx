...
interface PurchasesRincianClientProps {
  purchasesData: any[]
  branches: any[]
  suppliers: any[]
  initialFrom: string
...
export default function PurchasesRincianClient({
  purchasesData,
  branches,
  suppliers,
  initialFrom,
  ...
  const applyFilter = () => {
    setShowModal(false)
    let url = `/reports/purchases/rincian?from=${fromDate}&to=${toDate}`
    ...
    purchasesData.forEach(purchase => {
      purchase.purchase_items?.forEach((item: any) => {
        items.push({
          ...
          payment_status: purchase.payment_status === 'lunas' ? 'Lunas' : 'Tempo',
        })
      })
    })
    return items
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `Rincian_Faktur_Pembelian_${initialFrom}_${initialTo}.csv`)
    ...
    <div className="bg-slate-200 min-h-screen flex flex-col items-center py-8 relative font-sans print:bg-white print:py-0 print:block">
      ...
      <div className="bg-white shadow-xl max-w-[1400px] w-full mx-4 min-h-[800px] p-12 print:shadow-none print:m-0 print:max-w-none print:p-0">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-[#800000] mt-1">Rincian Faktur Pembelian</h1>
        ...
        <div className="w-full">
          {(!initialFrom || !initialTo) ? (
            ...
            <div className="text-center py-20 text-dark-400 border-t border-b border-dark-200">
              Tidak ada data pembelian pada rentang tanggal dan parameter ini.
            </div>
          ) : (
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-t-2 border-b-2 border-dark-900 text-dark-900">
                  ...
                  <th className="py-3 px-2 font-bold text-right whitespace-nowrap">Harga Beli</th>
                  <th className="py-3 px-2 font-bold text-right whitespace-nowrap">Total Harga</th>
                ...
                {detailedItems.map((item, idx) => (
                  <tr key={`${item.id}-${idx}`} className="hover:bg-slate-50 print:hover:bg-transparent transition-colors">
                    ...
                    <td className="py-3 px-2 align-top text-center">
                      <span className={cn("px-2 py-1 rounded-full text-[10px] font-bold uppercase", item.payment_status === 'Lunas' ? 'bg-success-100 text-success-700' : 'bg-warning-100 text-warning-700 print:border print:border-warning-500')}>
                        {item.payment_status}
                      </span>
                    </td>
                  </tr>
