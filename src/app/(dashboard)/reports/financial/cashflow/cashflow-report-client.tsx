...
import { ArrowLeft, Printer, Download, RefreshCw, Settings2, X, Calendar, MapPin, ChevronLeft } from 'lucide-react'
...
export default function CashflowReportClient({
  ...
  const applyFilter = () => {
    setShowModal(false)
    let url = `/reports/financial/cashflow?from=${fromDate}&to=${toDate}`
  ...
  const exportToCSV = () => {
    const headers = ['Tanggal', 'Referensi', 'Keterangan', 'Metode', 'Uang Masuk', 'Uang Keluar', 'Saldo Berjalan']
    const csvData = timelineData.map(t => [
    ...
    link.setAttribute('download', `Laporan_Arus_Kas_${initialFrom}_${initialTo}.csv`)
    ...
    <div className="bg-slate-200 min-h-screen flex flex-col items-center py-8 relative font-sans print:bg-white print:py-0 print:block">
      ...
      <div className="bg-white shadow-xl max-w-[1200px] w-full mx-4 min-h-[800px] p-12 print:shadow-none print:m-0 print:max-w-none print:p-0">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-[#800000] mt-1">Laporan Arus Kas (Buku Kas)</h1>
        ...
        <div className="w-full">
          {(!initialFrom || !initialTo) ? (
            ...
            <div className="text-center py-20 text-dark-400 border-t border-b border-dark-200">
              Tidak ada mutasi kas pada rentang tanggal dan parameter ini.
            </div>
          ) : (
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-t-2 border-b-2 border-dark-900 text-dark-900">
                  ...
                  <th className="py-3 px-2 font-bold">Referensi</th>
                  <th className="py-3 px-2 font-bold">Keterangan</th>
                  <th className="py-3 px-2 font-bold">Metode</th>
                  <th className="py-3 px-2 font-bold text-right text-success-700">Uang Masuk</th>
                  <th className="py-3 px-2 font-bold text-right text-danger-700">Uang Keluar</th>
                  <th className="py-3 px-2 font-bold text-right">Saldo Berjalan</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-dark-200 text-dark-700">
                {timelineData.map((t, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 print:hover:bg-transparent transition-colors">
                    <td className="py-3 px-2 whitespace-nowrap">{new Date(t.date).toLocaleString('id-ID')}</td>
                    <td className="py-3 px-2 font-mono text-primary-600">{t.ref}</td>
                    <td className="py-3 px-2">{t.description}</td>
                    <td className="py-3 px-2 uppercase text-[10px] font-bold">{t.method}</td>
                    <td className="py-3 px-2 text-right text-success-600">{t.inflow > 0 ? formatRupiah(t.inflow).replace('Rp', '') : '-'}</td>
                    <td className="py-3 px-2 text-right text-danger-600">{t.outflow > 0 ? formatRupiah(t.outflow).replace('Rp', '') : '-'}</td>
                    <td className="py-3 px-2 text-right font-semibold">{formatRupiah(t.balance).replace('Rp', '')}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-b-2 border-dark-900 font-bold text-dark-900">
                  <td colSpan={4} className="py-3 px-2 text-right">TOTAL MUTASI</td>
                  <td className="py-3 px-2 text-right text-success-700">{formatRupiah(timelineData.reduce((sum, t) => sum + t.inflow, 0)).replace('Rp', '')}</td>
                  <td className="py-3 px-2 text-right text-danger-700">{formatRupiah(timelineData.reduce((sum, t) => sum + t.outflow, 0)).replace('Rp', '')}</td>
        ...
        <div className="fixed inset-0 bg-dark-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 print-hidden animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-slide-up">
            ...
            <div className="p-6 space-y-6">
              <div>
                <h4 className="text-lg text-dark-700 mb-4 border-b border-dark-200 pb-2">Tanggal Mutasi</h4>
