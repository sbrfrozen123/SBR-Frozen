...
const old_csv_headers = "const headers = ['Waktu Mulai', 'Waktu Selesai', 'Kasir', 'Barang Terjual', 'Modal Awal', 'Total Penjualan', 'Kas Akhir (Sistem)', 'Kas Akhir (Fisik)', 'Selisih', 'Status']";
const new_csv_headers = "const headers = ['No', 'Waktu Shift', 'Kasir', 'Nama Barang', 'Quantity', 'Modal Awal', 'Total Penjualan Cash', 'Total Penjualan Transfer/QRS', 'Jumlah Total', 'Selisih', 'Status']";
content = content.replace(old_csv_headers, new_csv_headers);

// 3. Update CSV Row
const old_csv_row = `      const start = format(new Date(s.start_time), 'dd/MM/yyyy HH:mm', { locale: localeID })
...
// 5. Update Table Body
const old_tbody = `                      <td className="min-w-[200px]">
...
const new_tbody = `                      <td className="min-w-[180px] align-top py-2">
                        {shift.shiftItems && shift.shiftItems.length > 0 ? (
